% s_bcicompIIIiva.m - main script file that applies the method to BCI
...
for jj=1:length(subjects)
  ...
  %% Train & test the classifier for various values of lambda
  for ii=1:length(lambda)
    fprintf('lambda=%g\n',lambda(ii));
    [W, bias, stat] = lrr2(Xtr, Ytr, lambda(ii));
    ...
    memo(jj,ii).out  = apply_lrr2(Xte, memo(jj,ii).cls);
...
for ii=1:prod(size(memo))
  loss(ii)=memo(ii).loss;
end
